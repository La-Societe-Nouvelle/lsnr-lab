...
#'
#' @details This function calls the latest available footprints for divisions and discount
#'  them in specified-year euro terms.
#'
...
#' @examples
#'
#' build_discounted_divisions_fpt("GHG",2019,2022)
#'
#' @export
...
build_discounted_divisions_fpt=function(indicator,startYear,endYear = as.numeric(format(Sys.Date(), "%Y"))-1 ,verbose = T)
{
...

  {
    ...
    {
      stop(paste0("Indice des prix à la consommation indisponible pour l'annee ", endYear))
    }

    if(startYear %in% price_data$TIME_PERIOD == F )
    {
      stop(paste0("Indice des prix à la consommation indisponible pour l'annee ", startYear))
    }
