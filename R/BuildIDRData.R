...
build_branches_nva_fpt_idr = function(selectedYear)
{
  ...
  else
  {
    ...
    for(x in 1:nrow(DataIDR)){
      ...
      DataIDR$FOOTPRINT[x] = DataIDR$GROSS_IMPACT[x] = quantile(values$`Remunerations horaires`,prob=0.9,na.rm=T)/quantile(values$`Remunerations horaires`,prob=0.1,na.rm=T)
    }
